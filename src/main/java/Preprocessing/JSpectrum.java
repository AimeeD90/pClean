...
public class JSpectrum {
    ...
    * */
    public void module2(Boolean isoReduction, Boolean chargeDeconv, Boolean ionsMerge, Boolean filter) {
        if (isoReduction) {
            /*for (Preprocessing.JPeak jPeak : getPeaks()) {
    ...
    /*private void doIonMarge(Boolean ionsMarge) {
        ArrayList<Preprocessing.JPeak> jPeaks = new ArrayList<Preprocessing.JPeak>();
        double previousMZ = getPeaks().get(0).getMz();
        double previousIntensity = getPeaks().get(0).getIntensity();
        ...
        for (int i = 1; i < getPeaks().size(); i++) {
            ...
            if (Math.abs(previousMZ - mz) < 0.01) {
                ...
                Preprocessing.JPeak jPeak = new Preprocessing.JPeak(previousMZ, previousIntensity, previousCharge);
                jPeaks.add(jPeak);
                previousMZ = mz;
                previousIntensity = intensity;
                previousCharge = charge;
                ...
                if (i == (getPeaks().size() - 1)) {
                    Preprocessing.JPeak jPeakLastOne = new Preprocessing.JPeak(previousMZ, previousIntensity, previousCharge);
                    jPeaks.add(jPeakLastOne);
                }
            }
        }
